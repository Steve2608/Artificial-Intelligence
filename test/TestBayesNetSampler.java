...
public class TestBayesNetSampler {
	...
	private static void initRAUNet() {
		final List<String> tpo = Arrays.asList("RA", "S", "RE", "HU", "PA", "CO", "CH");
		bn_rau = new BayesNetSampler(tpo);
		final NodeProbabilityTable ptRA = new NodeProbabilityTable("RA", null);
		ptRA.setProbability("", 0.1);
		final NodeProbabilityTable ptS = new NodeProbabilityTable("S", Arrays.asList("RA"));
		ptS.setProbability("0", 0.02);
		ptS.setProbability("1", 0.8);
		final NodeProbabilityTable ptRE = new NodeProbabilityTable("RE", Arrays.asList("RA"));
		ptRE.setProbability("0", 0.7);
		ptRE.setProbability("1", 0.01);
		final NodeProbabilityTable ptHU = new NodeProbabilityTable("HU", Arrays.asList("RE"));
		ptHU.setProbability("0", 0.3);
		ptHU.setProbability("1", 0.8);
		final NodeProbabilityTable ptPA = new NodeProbabilityTable("PA", null);
		ptPA.setProbability("", 0.3);
		final NodeProbabilityTable ptCO = new NodeProbabilityTable("CO", Arrays.asList("HU", "RE", "PA"));
		...
		final NodeProbabilityTable ptCH = new NodeProbabilityTable("CH", Arrays.asList("CO"));
	...
	public void TestRainbowAndParty() {
		...
		for (int i = 0; i < N; ++i) {
			final HashMap<String, Boolean> sample = bn_rau.getPriorSample();
			if (sample.get("RA")) {
				++countRainbow;
			}
			if (sample.get("PA")) {
